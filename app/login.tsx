...
export default function LoginScreen() {
    ...
    // NOTE: registration is now email/password; OTP flow removed
    ...
    const handleLogin = async () => {
        if (activeTab === 'resident') {
            // Email/password login for residents
            ...
            if (emailValid && password.length >= 6) {
                ...
                Alert.alert('Invalid', 'Please enter a valid email and password (min 6 chars)');
            }
        } else {
            ...
            if (officialUserId) {
                ...
                try {
                    let result;
                    if (Platform.OS === 'web') {
                        // create and pass an invisible reCAPTCHA verifier for web
                        const authModule: any = await import('firebase/auth');
                        const RecaptchaClass = authModule.RecaptchaVerifier as any;
                        const firebaseModule: any = await import('@/app/firebase');
                        const verifier = new RecaptchaClass('recaptcha-container', { size: 'invisible' }, firebaseModule.auth as any);
                        result = await authService.sendOTP(officialUserId, verifier);
                    } else {
                        result = await authService.sendOTP(officialUserId);
                    }
    ...
    const isResidentValid = !!email && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email) && password.length >= 6;
    ...
    const openRegisterModal = () => {
        ...
        setRegisterForm({ name: '', email: '', password: '', houseNo: '', address: '' });
        setRegisterErrors({ name: '', email: '', password: '', houseNo: '', address: '' });
    };
    ...
    const validateRegisterForm = () => {
        let isValid = true;
        const errors: any = { name: '', email: '', password: '', houseNo: '', address: '' };
                    ...
                    <View style={styles.formContainer}>
                        ...
                        <View style={styles.infoBox}>
                            ...
                            <Text style={styles.infoText}>
                                {activeTab === 'resident' ? 'Sign in with your email and password.' : 'An OTP will be sent to your registered contact'}
                            </Text>
                        </View>
                            ...
                            <View style={styles.modalContent}>
                                <View style={styles.modalHeader}>
                                    ...
                                    <TouchableOpacity onPress={closeRegisterModal}>
                                        ...
                                        <View style={styles.modalInputGroup}>
                                            <Text style={styles.modalLabel}>Email <Text style={{ color: 'red' }}>*</Text></Text>
                                            <TextInput
                                                style={styles.modalInput}
                                                placeholder="Enter email address"
                                                placeholderTextColor="#7A8A99"
                                                ...
                                                autoCapitalize="none"
                                            />
                                            {registerErrors.email ? <Text style={styles.errorText}>{registerErrors.email}</Text> : null}
                                        </View>
